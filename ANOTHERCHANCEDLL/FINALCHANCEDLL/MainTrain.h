...
EXPORT void getCorrelatedPairs_CPP(char args[], const char* trainPath) {
...
class VectorWrapper {
	std::vector<pair<pair<float, float>,float>> vec;
...
public:
	...
	void push_back(float f1, float f2, float f3) {
		vec.push_back({ { f1, f2 }, f3 });
	}
	...
	std::pair<pair<float, float>, float> getByIndex(int x) {
		return vec[x];
	}
...
EXPORT void* getByIndex(VectorWrapper* v, int index) {
	v->curPair[0] = v->getByIndex(index).first.first;
	v->curPair[1] = v->getByIndex(index).first.second;
	v->curPair[2] = v->getByIndex(index).second;
...
EXPORT void getCorrelatedPairsWithPointAndRadius_CPP(VectorWrapper* lineInfo, const char* trainPath) {
	HybridAnomalyDetector ad;
	TimeSeries ts(trainPath);
	ad.learnNormal(ts);
	std::vector<correlatedFeatures> cf = ad.getNormalModel();
	for (int i = 0; i < cf.size();i++) {
		lineInfo->push_back(cf[i].center->x, cf[i].center->y, cf[i].threshold);
	}
}
